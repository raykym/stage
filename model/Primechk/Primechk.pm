...
# Primechk->checkspvm; rewrite SPVM

# Primechk->factor;
#         ->factorres;  ref @result
# Primechk->divisor;
# Primechk->divisorspvm; rewrite SPVM
...
# Primechk->equation; # 履歴から逆に展開して式を作る。 ax + by = 1の形式でx,yを求める
...
sub checkspvm {
    ...
    # 事前確認  6n+1 or 6n+5を確認してからチェックする それ以外は　0:falseを返す	
    my $chk = $num % 6;
    if (($chk == 1) || ($chk == 5) ) {
        # 素数かどうかをチェックする
           $self->{result} = SPVM::Primechk::check($num);
           return;
    }
    $self->{result} = 0; # false
    return;
...
sub divisorspvm {
    ...
    my $num = say $self->{string}; # BigIntを解除 

    # 素因数分解して指数をかけ合わせた数を出して、配列個数を引き渡し
       $self->factor;
    my $res = $self->factorres;
    my %hash;
    my @nocomp = grep !$hash{$_}++ , @$res;  # 重複排除
    my @count;  # 重複個数
    my $cnt = 1;
       for my $i (@nocomp){
           my $c = 0;
       ...
       # 素数の数＋１を掛け合わせて約数の個数を求める
       for my $i (@count){
           $cnt = $cnt * ($i + 1);
       }

     my $spvmres = SPVM::Primechk::divisor($num,$cnt);
     my $array = $spvmres->get_elements;

        # 余分な0を削除
        @$array = grep { $_ != 0 } @$array; 

       $self->{divisorres} = $array;
   ...
   for my $i (@mod_trans_r){
    ...
    #      $context =~ s/$mod[0]/$mod[1]/g;    # 余りが一桁だと多重マッチしてしまう。上のforループが同じことをしようとしている
       #   say "before: $context";
          ...
          my $calchan = Calcchange->new($context);
             $calchan->calcchange;
